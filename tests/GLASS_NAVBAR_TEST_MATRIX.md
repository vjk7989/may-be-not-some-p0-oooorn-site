...
| Motion | Staggered rolling label | Every enhanced link contains one full-string, visually hidden semantic label plus two `aria-hidden` animated visual layers. Each visible character rolls vertically in reading order over exactly 449.344ms, with exactly 37.0048ms between consecutive non-whitespace characters (both are exactly 1.18× the current 380.8ms/31.36ms timing, or 1.3216× the accepted 340ms/28ms baseline). Fine-pointer hover and keyboard focus produce the same completed state, and no background-fill animation is present. |
...
| Performance | Bounded CSS motion | The effect uses CSS transforms and opacity only. For each label, character `n` starts at `n × 37.0048ms` and the final boundary is `(non-whitespace character count − 1) × 37.0048ms + 449.344ms`; the longest current label, “Contact Us,” therefore settles at exactly 745.3824ms. No animation library, remote asset, continuous loop, background-fill animation, or layout-property animation is added. |
...
- Time: applicable only to deterministic presentation timing — the transform
  duration must be exactly 449.344ms, consecutive non-whitespace delays exactly
  37.0048ms, the longest current label must settle at exactly 745.3824ms, and all
  timing must be disabled by reduced motion. The animation does not affect
  application state.
...
Before this timing refinement is implemented, the exact timing checks fail
because the current per-character roll uses the accepted 380.8ms duration and
31.36ms stagger rather than their exact 1.18× values. The already approved unboxed
surfaces, route order, current-page semantics, character structure, easing,
transforms, outer glass fallbacks, Sheet operation, no-JavaScript navigation,
reduced-motion behavior, and overflow coverage should remain green.
